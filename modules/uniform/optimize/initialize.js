...
function initialize(moduleOptions) {
  ...
  this.addPlugin(
    nodePath.resolve(__dirname, "uniform-optimize-nuxt-plugin.js")
  );
...
function addIntentManifestFetcherPlugins({
  ...
  moduleContainer.addPlugin({
    src: nodePath.resolve(
      __dirname,
      "intent-manifest-fetcher-plugin.server.js"
    ),
    mode: "server",
    options: {
  ...
  moduleContainer.addPlugin({
    src: nodePath.resolve(
      __dirname,
      "intent-manifest-fetcher-plugin.client.js"
    ),
    mode: "client",
    options: {
      // The plugin `options` object is JSON.stringify'd before being made available
...
function ensurePluginOrder({ nuxtApp }) {
  ...
  nuxtApp.hook("builder:extendPlugins", plugins => {
    ...
    const optimizePluginIndex = plugins.findIndex(
      plugin =>
        plugin.src &&
        plugin.src.toLowerCase().includes("uniform-optimize-nuxt-plugin")
    );
